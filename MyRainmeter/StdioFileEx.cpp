...
UINT CStdioFileEx::ProcessFlags(const CString& sFilePath, UINT& nOpenFlags)
{
	...
	if (nOpenFlags & CFile::typeText && !(nOpenFlags & CFile::modeCreate) && !(nOpenFlags & CFile::modeWrite ))
	{
		...
		if (GetFileEncoding(sFilePath) == CP_UTF8)
		{
...
/*static*/ bool CStdioFileEx::IsFileUnicode(const CString& sFilePath)
{
	...
	if (GetFileEncoding(sFilePath) == CP_UNICODE)
	{
		bIsUnicode = true;
	}
	return bIsUnicode;
...
// --------------------------------------------------------------------------------------------
//
//	CStdioFileEx::GetFileCode()
//
// --------------------------------------------------------------------------------------------
// Returns:    int
// Parameters: const CString& sFilePath
//
// Purpose:		Return the result of file encoding type
//				
// Notes:		None.
// Exceptions:	None.
//
int CStdioFileEx::GetFileEncoding( const CString& sFilePath )
{
	CFile				file;
	int				encodingPage = -1;
	wchar_t			cFirstChar;
	...
	if (file.Open(sFilePath, CFile::typeBinary | CFile::modeRead, &exFile))
	{
		...
		{
			if ( cFirstChar == (wchar_t)nUNICODE_BOM)
			{
				encodingPage = CP_UNICODE;
			}
			if (cFirstChar == (wchar_t)nUTF8_BOM)
			{
				encodingPage = CP_UTF8;
			}
		}
